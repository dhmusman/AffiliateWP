...
<?php
	...
	*/
	...
	/**
	 * Creates an AffiliateWP coupon object.
	 *
	 * Requires a coupon object or array generated by the integration.
	 *
	 * @param  array $args          An array of coupon template data, used to populate the new coupon.
	 * @param  int   $affiliate_id  Affiliate ID.
	 * @return bool                 Returns true if an AffiliateWP coupon object was created, otherwise false.
	 * @since  2.2
	 */
	public function create_coupon( $meta, $discount_id ) {
		...
		if ( edd_discount_exists( $discount_id ) ) {
		...
		if ( ! $discount ) {
			affiliate_wp()->utils->log( 'Affiliate_WP_EDD::create_coupon: Unable to retrieve EDD discount with given discount ID: ' . $discount_id . '. Provided data: ' . print_r( $meta, true ) );
			return false;
		}
