...
<?php
	 ...
	 * as well as whether or not the cisount is used as a coupon template.
	 *
	 * @access  public
	 * @param   array   $details      EDD discount arguments.
	...
	public function store_discount_affiliate( $details, $discount_id = 0 ) {
		...
		if( ! empty( $_POST['affwp_is_coupon_template'] ) && empty( $affiliate_id ) ) {
		...
		$is_template = ! empty( $_POST['affwp_is_coupon_template'] );
		...
		// Look to see if we have an existing template
		$existing_template = affwp_get_coupon_template_id( $this->context );

		if( $existing_template && (int) $existing_template !== (int) $discount_id ) {

			// Remove the template flag from the existing template
			delete_post_meta( $existing_template, 'affwp_is_coupon_template' );
		...
		if( $existing_coupon ) {
			...
			affiliate_wp()->coupons->update( $existing_coupon->coupon_id, $args );
